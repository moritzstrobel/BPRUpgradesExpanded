...
PATTERN = {
    'breacher': (
        'Breaching Barrel',
        'Built for room clearing: 16 buckshot pellets and a much wider pattern, but 15% shorter effective falloff distance.',
        2800,
        ['BPRUE_SG_FractionCount16Effect', 'DispersionNeg40Effect', 'BPRUE_SG_DistanceDropOffNeg15Effect'],
    ),
    'field_choke': (
        'Field Choke',
        'A versatile choke keeping the standard 12-pellet load while tightening spread by 15% and extending falloff distance by 10%.',
        3000,
        ['BPRUE_SG_FractionCount12Effect', 'DispersionPos15Effect', 'DistanceDropOffLengthPos10Effect'],
    ),
    'full_choke': (
        'Full Choke',
        'A long-range setup using 10 pellets in a 30% tighter pattern with 20% longer falloff distance.',
        3200,
        ['BPRUE_SG_FractionCount10Effect', 'DispersionPos30Effect', 'DistanceDropOffLengthPos20Effect'],
    ),
}
...
ACTION = {
    'high_speed': (
        'High-Speed Action',
        'Cuts the firing cycle by 20%, but increases recoil by 15% and weapon wear by 20%.',
        3200,
        ['BPRUE_FireIntervalNeg20Effect', 'BPRUE_SG_RecoilPenalty15Effect', 'BPRUE_DurabilityPerShotNeg20Effect'],
    ),
    'tuned': (
        'Tuned Action',
        'Cuts the firing cycle by 10% and reduces recoil by 10%, at the cost of 10% increased weapon wear.',
        3000,
        ['BPRUE_FireIntervalNeg10Effect', 'RecoilDown10Effect', 'BPRUE_DurabilityPerShotNeg10Effect'],
    ),
    'reinforced': (
        'Reinforced Action',
        'Slows the firing cycle by 10% in exchange for 15% less recoil and 20% faster recoil recovery.',
        3000,
        ['BPRUE_SG_FireIntervalPos10Effect', 'RecoilDown15Effect', 'ShotRecoveryPos20Effect'],
    ),
}
...
HANDLING = {
    'lightweight': (
        ...
        'Cuts weapon weight by 15% and speeds aiming by 15%, but increases recoil by 15%.',
        2800,
        ['WeightDown15Effect', 'AimingTimePos15Effect', 'BPRUE_SG_RecoilPenalty15Effect'],
    ),
    'combat': (
        'Combat Furniture',
        'Improves close-range handling with 10% faster aiming, 10% faster movement while aiming and 10% faster recoil recovery, but increases weapon wear by 10%.',
        3000,
        ['AimingTimePos10Effect', 'AimingMovementPos10Effect', 'ShotRecoveryPos10Effect', 'BPRUE_DurabilityPerShotNeg10Effect'],
    ),
    'stabilized': (
        'Stabilized Furniture',
        'Reduces recoil by 15% and improves recoil recovery by 20%, but makes the weapon 5% heavier and aiming 10% slower.',
        3200,
        ['RecoilDown15Effect', 'ShotRecoveryPos20Effect', 'BPRUE_SG_WeightPos5Effect', 'AimingTimeNeg10Effect'],
    ),
}
...
def sid(prefix: str, group: str, key: str) -> str:
    return f'{prefix}_Upgrade_BPRUE_SG_{group.title()}_{key.title().replace("_", "")}'


def modules_for(name: str, fam: dict) -> list[dict]:
    out: list[dict] = []
    scale = fam.get('cost_scale', 1)
    for group, defs in [('Pattern', PATTERN), ('Action', ACTION), ('Handling', HANDLING)]:
        keys = list(defs)
        for key in keys:
            title, hint, cost, effects = defs[key]
            out.append({
                'sid': sid(fam['prototype_prefix'], group, key),
                'group': group,
                'key': key,
                'title': title,
                'hint': hint,
                ...
                'effects': effects,
                'block': [sid(fam['prototype_prefix'], group, k) for k in keys if k != key],
            })
    return out


def render_upgrade(config: dict) -> str:
    lines = [
        '// AUTO-GENERATED - Source: shotgun_upgrades.json',
        '',
        'BPRUE_ShotgunModuleTemplate : struct.begin {refurl=@BaseGame/UpgradePrototypes.cfg;refkey=[0]}',
        '   SID = BPRUE_ShotgunModuleTemplate',
        '   IsModification = true',
        'struct.end',
        '',
    ]
    for name, fam in config['families'].items():
        for m in modules_for(name, fam):
            ...
            lines += [
                f"{m['sid']} : struct.begin {{refkey=BPRUE_ShotgunModuleTemplate}}",
                f"   SID = {m['sid']}",
                f"   Text = sid_bprue_sg_{m['group'].lower()}_{m['key']}_name",
                f"   Hint = sid_bprue_sg_{m['group'].lower()}_{m['key']}_description",
                ...
                f"   UpgradeTargetPart = {target}",
                '   EffectPrototypeSIDs : struct.begin',
            ]
            lines += [f'      [{i}] = {e}' for i, e in enumerate(m['effects'])]
            lines += ['   struct.end', '   BlockingUpgradePrototypeSIDs : struct.begin']
            lines += [f'      [{i}] = {b}' for i, b in enumerate(m['block'])]
...
def render_weapons(config: dict) -> str:
    ...
    for name, fam in config['families'].items():
        lines += [
            f"{fam['general_setup_sid']} : struct.begin {{bpatch}}",
            '   UpgradePrototypeSIDs : struct.begin {bpatch}',
        ]
        for m in modules_for(name, fam):
            lines.append(f"      [*] = {m['sid']}")
        lines += ['   struct.end', 'struct.end', '']
    return '\n'.join(lines)
...
def main() -> None:
    ...
    UPGRADE_OUTPUT.write_text(render_upgrade(c), encoding='utf8')
    EFFECT_OUTPUT.write_text(render_effects(), encoding='utf8')
    WEAPON_OUTPUT.write_text(render_weapons(c), encoding='utf8')
