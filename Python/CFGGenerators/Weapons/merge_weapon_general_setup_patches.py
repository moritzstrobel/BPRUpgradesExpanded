...
def render_complete_upgrade_child(vanilla_sids: list[str], bprue_sids: list[str]) -> list[str]:
    ...
    # IMPORTANT: do not bpatch the nested array here. A nested {bpatch} keeps the
    # original indexed vanilla entries and then adds our SID-keyed copies, which
    # makes every vanilla upgrade appear twice in game. The top-level GeneralSetup
    # is already a bpatch; this child intentionally replaces UpgradePrototypeSIDs
    # with one complete, deterministic Vanilla+BPRUE array.
    lines = ["   UpgradePrototypeSIDs : struct.begin"]
    lines.extend(f"      [{index}] = {sid}" for index, sid in enumerate(combined))
...
def main() -> None:
    ...
    lines = [
        ...
        "// UpgradePrototypeSIDs replace the nested vanilla array with one complete",
        "// indexed Vanilla + BPRUE set sourced from Python/VanillaReference.",
