...
def render_upgrade(upgrade: dict, image_root: str) -> str:
    ...
    if upgrade.get("is_modification", False):
        lines.append("   IsModification = true")

    lines.append(f"   UpgradeTargetPart = {upgrade['target_part']}")
    ...
    lines += render_string_array("InterchangeableUpgradePrototypeSIDs", upgrade.get("interchangeable_upgrade_sids", []))
    lines += render_string_array("ConnectionLines", upgrade.get("connection_lines", []))
    lines.append("struct.end")
    return "\n".join(lines)


def render_effect_patch() -> str:
    return """// -----------------------------------------------------------------------------
...
BPRUE_FireRateUp10Effect : struct.begin {refkey=[0]}
   SID = BPRUE_FireRateUp10Effect
...
BPRUE_ReloadingTimeDecBy25Effect : struct.begin {refkey=[0]}
   SID = BPRUE_ReloadingTimeDecBy25Effect
...
// Vanilla has add/remove effects for 5.45 and 5.56, but no matching remove-7.62 effect.
// This mirrors ChangeAmmoTypesNo545Effect so switching away from the 7.62 test conversion is clean.
...
def render_weapon_patch(weapon: dict, upgrades: list[dict]) -> str:
    ...
    if "fire_queue_count" in weapon:
        lines.append(f"   FireQueueCount = {weapon['fire_queue_count']}")
...
def main() -> None:
    ...
    rendered_upgrades = [
        ...
        "// Includes three interchangeable standalone caliber-conversion test nodes.",
        "",
    ]
    for upgrade in upgrades:
    ...
    EFFECT_OUTPUT_PATH.write_text(render_effect_patch(), encoding="utf-8")
