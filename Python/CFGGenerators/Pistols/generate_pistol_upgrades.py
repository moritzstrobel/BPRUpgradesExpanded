...
def expanded_families(config: dict):
    ...
    their own GeneralSetup and reuse only the base family's regular module
    definitions and cost scale. Unique signatures are layered exclusively by the
    central unique weapon system.
    """
...
// Pistol-specific effects required by the shared specialization groups.
...
def main() -> None:
    ...
    outputs = {UPGRADE_OUTPUT: render_upgrade_prototypes(model, source="pistol_upgrades.json", template_sid=TEMPLATE_SID,
        header_comments=("Two mutually-exclusive three-way specialization groups; unique variants reuse their base-family modules with unique prototype SIDs.",)),
