...
def build_upgrades(config: dict) -> list[UpgradeDefinition]:
    ...
    for setup,(current,text,hint,cost) in PISTOL_CONVERSIONS.items():
        upgrades.append(UpgradeDefinition(sid=current,general_setup_sid=setup,weapon_class="SMG",group="Conversion",target_part="Body",text_sid=text,hint_sid=hint,image=IMAGE,icon=CALIBER_ICON,cost=cost,template_sid=TEMPLATE_SID,require_effects=False,standalone=True,module_image=PISTOL_CONVERSION_ICON))
    return upgrades
