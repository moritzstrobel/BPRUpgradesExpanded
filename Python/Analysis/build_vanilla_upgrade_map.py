...
def parse_upgrade_layout() -> dict[str, dict[str, object]]:
    ...
    for block in _top_level_blocks(VANILLA_UPGRADES.read_text(encoding="utf-8")):
        ...
        result[sid] = {"target_part": _enum_tail(target), "horizontal_position": horizontal,
                       "horizontal_explicit": horizontal_raw is not None, "vertical_position": _enum_tail(vertical),
                       "vertical_explicit": vertical is not None, "is_modification": is_modification}
    return result
...
def build_map() -> dict[str, object]:
    ...
    for setup_sid, upgrade_sids in sorted(setup_upgrades.items()):
        ...
        for upgrade_sid in upgrade_sids:
            ...
            slots[str(layout["target_part"])][int(layout["horizontal_position"])][str(layout["vertical_position"])].append({
                "sid": upgrade_sid, "is_modification": layout["is_modification"],
                "horizontal_explicit": layout["horizontal_explicit"], "vertical_explicit": layout["vertical_explicit"]})
        weapons[setup_sid] = {"upgrade_count": len(upgrade_sids), "slots": {
    ...
    return {"sources": {"upgrade_prototypes": str(VANILLA_UPGRADES.relative_to(PYTHON_ROOT)),
                         "general_setups": str(VANILLA_GENERAL_SETUPS.relative_to(PYTHON_ROOT))},
            "weapon_count": len(weapons), "weapons": weapons, "unresolved_upgrade_sids": unresolved}
...
def main() -> None:
    ...
    if args.weapon:
        print(); print_weapon(data, args.weapon)
    elif args.all:
        for setup_sid in data["weapons"]:
            print(); print_weapon(data, setup_sid)
