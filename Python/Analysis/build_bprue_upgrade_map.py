    ...
    for setup_sid, upgrades in sorted(setups.items()):
        ...
        for upgrade in upgrades:
            ...
            slots[upgrade.target_part][horizontal][vertical].append({
                "sid": upgrade.sid, "class": upgrade.weapon_class, "group": upgrade.group,
                "is_modification": True, "horizontal_explicit": upgrade.horizontal_position is not None,
                "vertical_explicit": upgrade.vertical_position is not None, "blocking_sids": list(upgrade.blocking_sids),
            })
        weapons[setup_sid] = {
            "content_pack": content_pack or "BaseGame",
            "upgrade_count": len(upgrades),
            "slots": {target: {str(column): dict(sorted(verticals.items())) for column, verticals in sorted(columns.items())} for target, columns in sorted(slots.items())},
        }
    return weapons
...
def build_map() -> dict[str, object]:
    ...
    weapons = _weapon_map(model, content_pack=None)
    pack_counts = {"BaseGame": len(weapons)}
    total_upgrades = len(model.upgrades)
    for pack, dlc_model in sorted(dlc_models.items()):
        mapped = _weapon_map(dlc_model, content_pack=pack)
        duplicates = set(weapons) & set(mapped)
        if duplicates:
            raise ValueError(f"Duplicate GeneralSetup SIDs across output scopes: {', '.join(sorted(duplicates))}")
        weapons.update(mapped)
        pack_counts[pack] = len(mapped)
        total_upgrades += len(dlc_model.upgrades)
    return {
        "source": "base/Unique plus DLC UpgradeBuildModels after apply_layout_to_model()",
        "weapon_count": len(weapons), "upgrade_count": total_upgrades,
        "content_pack_counts": pack_counts, "weapons": dict(sorted(weapons.items())),
    }
...
def main() -> None:
    ...
    print(f"Built BPRUE upgrade map for {data['weapon_count']} GeneralSetups / {data['upgrade_count']} upgrades ({packs})")
